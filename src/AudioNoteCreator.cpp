    ...
    m_recordingAmplitude(0.0)
{
  ...
  constexpr int fiveSecond = 5000;
  updateDevicesTimer->start(fiveSecond);
...
{
  return !m_audioNote->name().isEmpty() && !m_recordedPath.isEmpty() && !(m_audioNote->encrypted() &&
         m_audioNote->password().isEmpty());
}
